use crate::app::types::{
    AppError, AppEvent, AppResult, AudioPlayerEvent, FinderEvent, UIEvent, ViewEvent,
};
...
impl<View: AppView> Application<View> {
    ...
    fn finder_event_handler(&mut self, event: FinderEvent) {
        match event {
            FinderEvent::NewFiles(files) => match self.player.try_new_playlist(files) {
        ...
        match event {
            UIEvent::PlayPause(id) => {
                match self.player.play_pause(id) {
                    ...
                    Ok(None) => self.view.update(ViewEvent::PlayPause),
                    Err(err) => self.error_event_handler(err),
                };
            }
            UIEvent::Next => {
