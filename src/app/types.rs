use thiserror::Error;
use crate::player::playlist::{Track, TrackMaybeId, TrackId, TrackProgress, TrackMetadata};
...
impl PartialEq for AppEvent {
    fn eq(&self, other: &Self) -> bool {
        match (self, other) {
            (AppEvent::UI(_), AppEvent::UI(_)) => true,
            (AppEvent::Player(_), AppEvent::Player(_)) => true,
            (AppEvent::Finder(_), AppEvent::Finder(_)) => true,
            (AppEvent::Error(_), AppEvent::Error(_)) => true,
...
impl SenderExt for Sender<AppEvent> {
    ...
    fn send_error_event(&self, err: AppError) { self.send(AppEvent::Error(err)).unwrap(); }
